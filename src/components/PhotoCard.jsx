    ...
    >
      ...
      {error && (
        <div className="w-full aspect-square flex flex-col items-center justify-center bg-ash/50 text-white/30 gap-2">
          ...
          <p className="text-xs text-center px-4">Photo not found<br /><span className="text-gold/60">{photo.file}</span></p>
        </div>
      )}
      ...
      {!error && (
        <img
          ...
          className={`w-full aspect-square object-cover transition-all duration-500 group-hover:scale-105 ${loaded ? "opacity-100" : "opacity-0 absolute inset-0"}`}
        />
      )}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
        <p className="font-display font-semibold text-soft-white text-sm truncate">{photo.title}</p>
        {photo.date && (
          <p className="text-white/50 text-xs mt-0.5">{new Date(photo.date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p>
        )}
      </div>
    </div>
  );
